...
export function MapView() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Higher tilt (80°) positions user in lower third, showing maximum area ahead
      map3d.setAttribute('tilt', '80');
    ...
    <div className="relative w-screen h-screen overflow-hidden" style={{ background: '#1a1a1a' }}>
      ...
      <div className="absolute top-24 right-4 z-20 flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-1">
                <button
                  onClick={() => {
                    if (mapRef.current) {
                      const map3d = mapRef.current as any;
                      const currentTilt = parseFloat(map3d.getAttribute('tilt') || '80');
        ...
        {debugMode && (
          ...
          >
            {(() => {
              const currentTilt = parseFloat((mapRef.current as any)?.getAttribute('tilt') || '80');
      ...
      {debugMode && (
        <div className="absolute top-4 left-4 z-50 bg-black/80 text-white text-xs p-3 rounded-lg backdrop-blur-sm max-w-xs">
          ...
          <div>Tilt: {(mapRef.current as any)?.getAttribute('tilt') || '80'}°</div>
